...
set abc_exec "yosys-abc"
if {[info exists ::env(YOSYS_ABC)] && $::env(YOSYS_ABC) ne ""} {
	set abc_exec $::env(YOSYS_ABC)
}
...
foreach fn [glob *.sv] {
	...
	foreach m [module_list] {
		...
		file delete $flag
		yosys exec -- $abc_exec -c "\"read $fn-$m.aig; pdr -v; write_cex '$fn-$m.aig.flag'\""
